...
final class CoursController extends AbstractController
{
    ...
    #[Route(path: '/calendar', name: 'app_cour_calendar', methods: ['GET'])]
    public function calendar(CoursRepository $coursRepository): Response
    {
        // Fetch courses and prepare events array
        ...
        foreach ($courses as $cours) {
            ...
            $heureFin = clone $heureDebut; // Clone to avoid modifying the original heureDebut
    
            // Parse duration (e.g., "2heures" or "4heures")
            if (preg_match('/(\d+)\s*heures/i', $duree, $matches)) {
                $hoursToAdd = (int) $matches[1];
                $heureFin->modify("+$hoursToAdd hour");
            }
    
            $events[] = [
                'title' => $cours->getNom(),
                'start' => $cours->getDate()->format('Y-m-d') . 'T' . $heureDebut->format('H:i:s'),
                'end' => $cours->getDate()->format('Y-m-d') . 'T' . $heureFin->format('H:i:s'),
            ];
        }
